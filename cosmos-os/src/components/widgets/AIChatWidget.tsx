...
export const AIChatWidget: React.FC = () => {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-[#1a1c29]/50 text-white font-sans p-3">
      {/* Header */}
      <div className="flex items-center gap-2 pb-2 mb-2 border-b border-white/10">
        <span className="text-xl">🤖</span>
        <div className="flex flex-col">
          <span className="text-[13px] font-bold text-[#3b82f6] leading-tight">COSMOS AI</span>
          <span className="text-[9px] text-white/50 leading-tight">Always here to help</span>
        </div>
      </div>

      {/* Chat Area */}
      <div className="flex-1 overflow-y-auto overflow-x-hidden pr-1 custom-scrollbar flex flex-col gap-2">
        {messages.map(msg => (
          <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div 
              className={`max-w-[85%] px-3 py-1.5 rounded-2xl text-[11px] leading-relaxed shadow-sm ${
                msg.sender === 'user' 
        ...
        {isTyping && (
          <div className="flex justify-start">
            <div className="bg-white/10 px-3 py-2 rounded-2xl rounded-bl-sm border border-white/5 flex gap-1 items-center">
      ...
      <div className="pt-2 mt-2 border-t border-white/10 flex gap-1.5">
        <input 
          ...
          placeholder="Ask me to open an app..." 
          className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-1.5 text-[11px] text-white placeholder-white/30 outline-none focus:bg-white/10 focus:border-white/20 transition-all"
        />
        <button 
          onClick={handleSend}
          disabled={!inputValue.trim()}
          className="w-8 h-8 flex items-center justify-center bg-[#3b82f6]/90 hover:bg-[#3b82f6] disabled:opacity-50 disabled:hover:bg-[#3b82f6]/90 rounded-xl transition-colors text-white shadow-md"
        >
          ➤
        </button>
      </div>
